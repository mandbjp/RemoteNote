...
class Mouse:
    ...
    def _do_event(self, flags, x_pos, y_pos, data, extra_info):
        ...
        return ctypes.windll.user32.mouse_event(flags, DWORD(int(x_calc)), DWORD(int(y_calc)), data, extra_info)
    ...
    def wheel(self, delta):
        """let wheel mouse to the specified coordinates. positive value goes up"""
        self._do_event(self.MOUSEEVENTF_WHEEL, 0, 0, delta, 0)

    def wheel_horizontal(self, delta):
        """let wheel mouse to the specified coordinates. positive value goes right"""
    ...
    def get_position(self):
        ...
        ret = ctypes.windll.user32.GetCursorPos(ctypes.byref(pt))
        if not ret:
            return None
        return pt.x, pt.y